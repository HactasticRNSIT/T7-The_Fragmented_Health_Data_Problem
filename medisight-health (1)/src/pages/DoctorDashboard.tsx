      ...
      {loading ? (
        ...
        <div className="dash-content">
          ...
          {activeTab === 'profile' && selectedPatient && (
            <div className="profile-view animate-fade-in" style={{ padding: '20px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '32px' }}>
                ...
                <h2 style={{ margin: 0 }}>Patient Profile: {selectedPatient.name}</h2>
              </div>

              <div className="dash-grid-2" style={{ gridTemplateColumns: 'minmax(300px, 400px) 1fr', gap: '32px' }}>
                {/* Left Column: Personal Info */}
                <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
                  <div className="dash-card" style={{ padding: '32px', textAlign: 'center' }}>
                    <div className="user-avatar" style={{ width: 100, height: 100, fontSize: '2rem', margin: '0 auto 20px', background: 'var(--primary)', color: 'white' }}>
                      {selectedPatient.name.substring(0, 2).toUpperCase()}
                    </div>
                    <h3 style={{ marginBottom: '4px' }}>{selectedPatient.name}</h3>
                    <p style={{ color: 'var(--text-muted)', marginBottom: '24px' }}>Patient ID: P-{selectedPatient.id.substring(0, 8)}</p>
                    
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', textAlign: 'left' }}>
                      <div style={{ padding: '12px', background: 'var(--surface-dim)', borderRadius: '12px' }}>
                        <p style={{ fontSize: '0.7rem', color: 'var(--text-muted)', margin: '0 0 4px 0' }}>AGE</p>
                        <p style={{ margin: 0, fontWeight: 700 }}>{selectedPatient.age} Years</p>
                      </div>
                      <div style={{ padding: '12px', background: 'var(--surface-dim)', borderRadius: '12px' }}>
                        <p style={{ fontSize: '0.7rem', color: 'var(--text-muted)', margin: '0 0 4px 0' }}>BLOOD GROUP</p>
                        <p style={{ margin: 0, fontWeight: 700, color: '#b30000' }}>{selectedPatient.bloodGroup}</p>
                      </div>
                    </div>
                  </div>

                  <div className="dash-card">
                    <h4 style={{ marginBottom: '16px', fontSize: '0.9rem', color: 'var(--text-muted)' }}>CONTACT INFORMATION</h4>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                      <div>
                        <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)', margin: 0 }}>Email Address</p>
                        <p style={{ margin: 0, fontSize: '0.9rem' }}>{selectedPatient.name.toLowerCase().replace(' ', '.')}@email.com</p>
                      </div>
                      <div>
                        <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)', margin: 0 }}>Phone Number</p>
                        <p style={{ margin: 0, fontSize: '0.9rem' }}>+1 (555) 012-3456</p>
                      </div>
                      <div>
                        <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)', margin: 0 }}>Emergency Contact</p>
                        <p style={{ margin: 0, fontSize: '0.9rem' }}>Sarah Johnson (Spouse)</p>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
                  <div className="dash-card">
                    <h3 style={{ marginBottom: '24px', display: 'flex', alignItems: 'center', gap: '10px' }}>
                      <FileText size={20} color="var(--accent)" /> Medical Documents
                    </h3>
                    <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem', marginBottom: '20px' }}>
                      Historical records, lab results, and imaging scans associated with this patient.
                    </p>
                    
                    <div style={{ display: 'grid', gap: '12px' }}>
                      {[
                        { name: 'Blood Test Results - Full Panel.pdf', date: 'Oct 24, 2025', size: '1.2 MB', type: 'PDF' },
                        { name: 'Chest X-Ray Digital Scan.jpg', date: 'Sep 12, 2025', size: '4.5 MB', type: 'IMG' },
                        { name: 'Cardiac Stress Test Summary.pdf', date: 'Aug 05, 2025', size: '840 KB', type: 'PDF' },
                        { name: 'Patient History Form.docx', date: 'Jul 20, 2025', size: '45 KB', type: 'DOC' },
                        { name: 'MRI Scan - Lumbar Spine.zip', date: 'May 18, 2025', size: '128 MB', type: 'ZIP' }
                      ].map((doc, i) => (
                        <div key={i} style={{ 
                          display: 'flex', 
                          alignItems: 'center', 
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                            ...
                            <div>
                              <p style={{ margin: 0, fontWeight: 600, fontSize: '0.9rem' }}>{doc.name}</p>
                              <p style={{ margin: 0, fontSize: '0.75rem', color: 'var(--text-muted)' }}>Uploaded on {doc.date} • {doc.size}</p>
                            </div>
                          </div>
                          <button style={{ color: 'var(--accent)', background: 'none', border: 'none', cursor: 'pointer', fontSize: '0.85rem', fontWeight: 600 }}>Download</button>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="dash-card">
                    <h3 style={{ marginBottom: '24px', display: 'flex', alignItems: 'center', gap: '10px' }}>
                      <History size={20} color="var(--accent)" /> Recent Consultations
                    </h3>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                      <div style={{ padding: '16px', borderLeft: '3px solid var(--accent)', background: 'rgba(0, 212, 204, 0.05)' }}>
                        <p style={{ fontSize: '0.75rem', fontWeight: 700, color: 'var(--accent)', marginBottom: '4px' }}>LATEST VISIT: {new Date(selectedPatient.lastVisit).toLocaleDateString()}</p>
                        <h4 style={{ margin: '0 0 8px 0' }}>{selectedPatient.lastReason}</h4>
                        <p style={{ margin: 0, fontSize: '0.85rem', color: 'var(--text-secondary)' }}>
                           Patient presented with recurring symptoms. Prescribed antibiotics and recommended 7 days of rest. Follow-up required if symptoms persist beyond 10 days.
                        </p>
                      </div>
                    </div>
                  </div>
