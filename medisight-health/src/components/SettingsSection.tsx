...
import { Moon, Sun, Type, Layout, Shield, Bell, Lock } from 'lucide-react';
import { useHealth } from '../context/HealthContext';

const SettingsSection: React.FC = () => {
  const { userProfile, toggleTheme, setFontSize } = useHealth();
  const settings = userProfile.settings || { theme: 'light', fontSize: 'medium' };

  return (
    <div className="settings-container animate-fade-in">
      <div style={{ marginBottom: '32px' }}>
        <h2 style={{ fontSize: '2rem', marginBottom: '8px', color: 'var(--text-primary)' }}>Appearance Settings</h2>
        <p style={{ color: 'var(--text-secondary)' }}>Customize how MediSight looks on your device.</p>
      </div>
